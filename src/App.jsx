...
import { useState, useEffect } from "react";
import { Activity, Clock, Menu, Radar, Satellite, ShieldAlert } from "lucide-react";
import { api } from "./api/client.js";
import { CommandDeck } from "./pages/CommandDeck.jsx";
import { Workbench } from "./pages/Workbench.jsx";
import { AlertLog } from "./pages/AlertLog.jsx";
...
const NAV = [
  { key: "deck", label: "Command Deck", icon: Satellite },
  { key: "workbench", label: "Analysis", icon: Radar },
  { key: "alerts", label: "Alert Log", icon: ShieldAlert },
];
  ...
  return (
    <div className="app-shell">
      ...
      <header className="mission-header">
        <button className="brand" onClick={() => setPage("deck")} aria-label="Open Command Deck">
          <span className="brand-mark"><Satellite size={15} /></span>
          <span>BORDERWATCH</span>
          <span className="brand-divider" />
          <small>ORBITAL INTELLIGENCE</small>
        </button>

        <nav className="mission-nav" aria-label="Primary navigation">
        ...
        <div className="mission-status">
          ...
          <span className="desktop-status"><Activity size={12} /> NOMINAL</span>
          <span className="clock"><Clock size={12} /> {now.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" })} UTC</span>
          <button className="menu-button" aria-label="Open menu"><Menu size={17} /></button>
        </div>
      </header>

      <main className={page === "deck" ? "mission-main deck-main" : "mission-main workspace-main"}>
        {page === "deck" && <CommandDeck alerts={alerts} aois={aois} onSelectSector={openSector} />}
        {page === "workbench" && aois.length > 0 && <Workbench aois={aois} selectedAoiId={selectedAoiId} />}
        {page === "workbench" && aois.length === 0 && <EmptyState title="Awaiting sector data" copy="Connect the analysis API to begin a comparison run." />}
        {page === "alerts" && <AlertLog alerts={alerts} />}
      </main>
    </div>
  );
...
function EmptyState({ title, copy }) {
  return <div className="empty-state"><Satellite size={24} /><h1>{title}</h1><p>{copy}</p></div>;
}
