...
export function OrbitalHUD({ globeRef, hostRef, lat, lon, label, onActivate }) {
  const [track, setTrack] = useState(null); const frame = useRef(null);
  useEffect(() => {
    const tick = () => {
      const globe = globeRef.current; const host = hostRef.current;
      if (globe && host && globe.getCoords && globe.camera) {
        const world = globe.getCoords(lat, lon, .02); const camera = globe.camera();
        if (world && camera) { const v = new THREE.Vector3(world.x, world.y, world.z); const facing = v.clone().normalize().dot(camera.position.clone().normalize()); v.project(camera); const x = (v.x + 1) * .5 * host.clientWidth; const y = (1 - v.y) * .5 * host.clientHeight; setTrack({ x, y, visible: facing > .15 && v.z < 1 }); }
      }
      frame.current = requestAnimationFrame(tick);
    }; frame.current = requestAnimationFrame(tick); return () => cancelAnimationFrame(frame.current);
  }, [globeRef, hostRef, lat, lon]);

  return <div className="orbital-hud" aria-hidden="true">
    <div className="coordinate-readout"><span>FOCUS / AOI-041</span><b>27.8300° N&nbsp;&nbsp;70.1700° E</b></div>
    {track && <><svg className="focus-link"><line x1={track.x} y1={track.y} x2="72%" y2="18%" /></svg><button className="focus-tag" style={{ opacity: track.visible ? 1 : 0 }} onClick={onActivate}><i /> {label}<small>SELECT SECTOR</small></button></>}
  </div>;
}
